...
-- same but using alias for table names
SELECT c.city_name, c.state_abbreviation, s.state_name
FROM  city AS c
JOIN state AS s ON s.state_abbreviation = c.state_abbreviation
...
SELECT p.park_name, s.state_name, s.capital
FROM park p
JOIN park_state ps ON p.park_id = ps.park_id
JOIN state s ON ps.state_abbreviation = s.state_abbreviation
...
SELECT p.park_name, s.state_name, s.capital, p.area
FROM park p
JOIN park_state ps ON p.park_id = ps.park_id
JOIN state s ON ps.state_abbreviation = s.state_abbreviation
JOIN city c ON s.capital = c.city_id;
...
SELECT s.state_name, MIN(p.date_established) AS earliest_date_park
FROM state s

using System.Collections.Generic;
using Microsoft.AspNetCore.Mvc;
using AuctionApp.Models;
using AuctionApp.DAO;

namespace AuctionApp.Controllers
{
    [Route("[controller]")]
    [ApiController]
    public class AuctionsController : ControllerBase
    {
        private readonly IAuctionDao dao;

        public AuctionsController(IAuctionDao auctionDao)
        {
            dao = auctionDao;
        }

        [HttpGet]
        public List<Auction> List(string title_like = "", double currentBid_lte = 0)
        {
            if (title_like != "")
            {
                return dao.SearchByTitle(title_like);
            }
            if (currentBid_lte > 0)
            {
                return dao.SearchByPrice(currentBid_lte);
            }

            return dao.List();
        }

        [HttpGet("{id}")]
        public ActionResult<Auction> Get(int id)
        {
            Auction auction = dao.Get(id);
            if (auction != null)
            {
                return Ok(auction);
            }
            else
            {
                return NotFound();
            }
        }

        [HttpPost]
        public ActionResult<Auction> Create(Auction auction)
        {
            Auction createdAuction = dao.Create(auction);
            return Created($"/{createdAuction.Id}", createdAuction);
            
        }

        [HttpPut("{auctionId}")]
        public ActionResult<Auction> Update(int auctionId, Auction auction)
        {
            Auction retrievedAuction = dao.Get(auctionId);

            if (retrievedAuction == null)
            {
               return NotFound("Auction not found");
            }
            else
            {
                return Ok(dao.Update(auctionId, auction));
            }

        }

        [HttpDelete("{auctionId}")]
        public ActionResult Delete(int auctionId)
        {
            if (dao.Delete(auctionId))
            {
                return NoContent();
            }
            else
            {
                return NotFound("Auction not found");
            }
        }

    }
}

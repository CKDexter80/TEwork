...
-- Add Disneyland to the park table. (It was established on 7/17/1955, has an area of 2.1 square kilometers and does not offer camping.)
	...
	VALUES ('Disneyland', '1955-07-17', 2.1, 0);
...
UPDATE state
SET capital = (SELECT city_id from city WHERE city_name = 'Sacramento'),
...
WHERE city_id = 
	(SELECT city_id FROM city WHERE city_name = 'Hawkins')
		AND state_abbreviation =
			(SELECT state_abbreviation FROM state WHERE state_name = 'Indiana');
